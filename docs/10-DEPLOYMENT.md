...
- 腾讯云 COS 私有桶
- 腾讯云短信（XW-1002 具体实现；“已批准短信服务”仅表示未来扩展能力）
- 8 模型 API
- DeepSeek 内容／评分
- 豆包图片
...
开发环境使用 Mock，不向真实手机号发送。XW-1002 的生产实现固定使用腾讯云短信
Cloud API 3.0 `SendSms`（版本 `2021-01-11`，固定端点
`sms.tencentcloudapi.com`），官方 Python SDK 固定为
`tencentcloud-sdk-python==3.1.141`。`SMS_REGION` 必须显式配置；内部不重试，
超时默认 10 秒，staging/production 禁止回退 Mock。验证码模板参数顺序固定为
`{1}=code`、`{2}=expiry_minutes`。生产还需配置已审核签名、模板 ID、频率和失败告警。
...
- 模型健康检查
- 额度一致性校验
- 管理员安全关键操作 SMS Step-Up（业务高风险动作及普通密码登录不默认发短信）
- 安全组和内网连接
- 备份和恢复测试
- 严重告警短信测试
- 内容审核流程测试
