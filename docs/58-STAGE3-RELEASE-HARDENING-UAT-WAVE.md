...
- real Tencent SMS credentials, approved templates/signature, delivery test, and administrator
  high-risk Step-Up capability (normal administrator password login does not require SMS);
...
The stable `sms` readiness check now describes the administrator high-risk Step-Up capability.
