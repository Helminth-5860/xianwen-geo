...
export POSTGRES_DB="${POSTGRES_DB:-cycle_reset_test_db}"
export POSTGRES_USER="${POSTGRES_USER:-cycle_reset_test_user}"
...
compose=(docker compose -f docker-compose.yml -f docker-compose.cycle-reset.yml)
cleanup() {
  "${compose[@]}" --project-name xianwen-cycle-reset-test --profile cycle-reset-test \
    down --volumes --remove-orphans
}
trap cleanup EXIT
cleanup
"${compose[@]}" --project-name xianwen-cycle-reset-test --profile cycle-reset-test \
  run --rm --build cycle-reset-tests
