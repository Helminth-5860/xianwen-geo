...
export POSTGRES_DB="${POSTGRES_DB:-subject_schema_test_db}"
export POSTGRES_USER="${POSTGRES_USER:-subject_schema_test_user}"
...
compose=(docker compose -f docker-compose.yml -f docker-compose.subject-schema.yml)
cleanup() {
  "${compose[@]}" --project-name xianwen-subject-schema-test --profile subject-schema-test \
    down --volumes --remove-orphans
}
trap cleanup EXIT
cleanup
"${compose[@]}" --project-name xianwen-subject-schema-test --profile subject-schema-test \
  run --rm --build subject-schema-tests
