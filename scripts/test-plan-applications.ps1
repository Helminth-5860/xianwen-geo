...
try {
    docker compose --project-name xianwen-plan-application-test --profile plan-application-test run --rm --build plan-application-tests
    if ($LASTEXITCODE -ne 0) { throw "PostgreSQL plan application tests failed." }
}
finally {
    docker compose --project-name xianwen-plan-application-test --profile plan-application-test down --volumes --remove-orphans
}
