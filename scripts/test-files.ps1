...
if ([string]::IsNullOrWhiteSpace($env:FILE_PROCESSING_DATABASE_URL)) { $env:FILE_PROCESSING_DATABASE_URL = "postgresql://$($env:POSTGRES_USER):$($env:POSTGRES_PASSWORD)@postgres:5432/test_$($env:POSTGRES_DB)" }
