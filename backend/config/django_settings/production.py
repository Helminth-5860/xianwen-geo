...
VIDEO_IDEMPOTENCY_HMAC_KEY = require_env("VIDEO_IDEMPOTENCY_HMAC_KEY")
...
if (
    len(VIDEO_IDEMPOTENCY_HMAC_KEY) < 50
    or VIDEO_IDEMPOTENCY_HMAC_KEY.lower() in weak_secret_markers
    or VIDEO_IDEMPOTENCY_HMAC_KEY.lower().startswith(("local-test-", "replace-"))
):
    raise ImproperlyConfigured("VIDEO_IDEMPOTENCY_HMAC_KEY is too weak for production.")
if VIDEO_IDEMPOTENCY_HMAC_KEY in {
    SECRET_KEY,
    SMS_VERIFICATION_HMAC_KEY,
    QUOTA_IDEMPOTENCY_HMAC_KEY,
    GEO_DETECTION_IDEMPOTENCY_HMAC_KEY,
    ...
    raise ImproperlyConfigured("VIDEO_IDEMPOTENCY_HMAC_KEY must be independent.")
if VIDEO_PROVIDER == "aliyun" and not (ALIYUN_VIDEO_API_BASE_URL and ALIYUN_VIDEO_API_KEY):
    raise ImproperlyConfigured(
        "ALIYUN_VIDEO_API_BASE_URL and ALIYUN_VIDEO_API_KEY are required "
        "when video generation is enabled."
    )
if (
...
if FILE_STORAGE_PROVIDER == "s3":
    ...
    if VIDEO_IDEMPOTENCY_HMAC_KEY == S3_SECRET_KEY:
        raise ImproperlyConfigured("VIDEO_IDEMPOTENCY_HMAC_KEY must not reuse S3_SECRET_KEY.")
...
for forbidden_variable in ("DATABASE_URL", "REDIS_URL"):
    ...
    if VIDEO_IDEMPOTENCY_HMAC_KEY in {configured_url, configured_password}:
        raise ImproperlyConfigured(
            f"VIDEO_IDEMPOTENCY_HMAC_KEY must not reuse {forbidden_variable} credentials."
        )
...
_release_secrets = {
    ...
    "VIDEO_IDEMPOTENCY_HMAC_KEY": VIDEO_IDEMPOTENCY_HMAC_KEY,
