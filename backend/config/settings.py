...
# 官网深度检测、自动发文和信源指数在所有环境共用同一套业务内核。
INSTALLED_APPS = [
    *INSTALLED_APPS,
    "apps.website_audits",
    "apps.publishing",
...
# 信源指数只使用百度公开搜索元数据，不抓取目标网页正文。5分钟是硬上限，不是目标耗时。
...
SOURCE_INDEX_SEARCH_BUDGET_SECONDS = int(os.getenv("SOURCE_INDEX_SEARCH_BUDGET_SECONDS", "240"))
