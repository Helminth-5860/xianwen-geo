...
# 官网深度检测、自动发文、信源指数和负面信息指数在所有运行环境共用同一套业务内核。
INSTALLED_APPS = [
    *INSTALLED_APPS,
    "apps.website_audits",
    "apps.publishing",
...
# 百度公开搜索发现层由信源指数和负面信息指数共享；信源指数自身仍只使用搜索元数据。
...
# 负面信息指数 V1：百度专项召回 + 结构化 AI 判定 + 少量高风险正文核验。
# 预算刻意低于 Celery 5 分钟硬上限，给分类、聚类和事务落库留出余量。
...
NEGATIVE_INDEX_VERIFICATION_TEXT_CHARS = int(os.getenv("NEGATIVE_INDEX_VERIFICATION_TEXT_CHARS", "12000"))
NEGATIVE_INDEX_MIN_NEGATIVE_CONFIDENCE = int(os.getenv("NEGATIVE_INDEX_MIN_NEGATIVE_CONFIDENCE", "60"))
...
if NEGATIVE_INDEX_SEARCH_BUDGET_SECONDS <= 0 or NEGATIVE_INDEX_SEARCH_BUDGET_SECONDS > NEGATIVE_INDEX_TOTAL_TIMEOUT_SECONDS - 90:
    raise ImproperlyConfigured("NEGATIVE_INDEX_SEARCH_BUDGET_SECONDS must leave at least 90 seconds for analysis.")
...
if NEGATIVE_INDEX_VERIFICATION_TEXT_CHARS < 2000 or NEGATIVE_INDEX_VERIFICATION_TEXT_CHARS > 20000:
    raise ImproperlyConfigured("NEGATIVE_INDEX_VERIFICATION_TEXT_CHARS must be between 2000 and 20000.")
