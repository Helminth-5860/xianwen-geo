...
def build_subject_search_context(subject: Subject) -> SubjectSearchContext:
    ...
    if version is not None:
        for value in SubjectName.objects.filter(subject_version=version).values_list("display_value", flat=True):
            append_unique(anchors, value)
    if not official_name and anchors:
        official_name = anchors[0]
    products: list[str] = []
    if version is not None:
        product_qs = SubjectProduct.objects.filter(subject_version=version).order_by("-include_in_mention", "display_value")
    ...
    if keyword_set is not None and keyword_set.current_version_id:
        keyword_qs = Keyword.objects.filter(keyword_set_version_id=keyword_set.current_version_id).order_by("sort_order", "id")
    ...
    return SubjectSearchContext(official_name=official_name, anchors=anchors, products=products, keywords=keywords, self_domains=self_domains)
