...
def run_adaptive_search(
    *,
    ...
    queue: deque[SearchTask] = deque(
        SearchTask(query=query) for query in initial_queries
    )
    ...
    while queue:
        ...
        if (
            elapsed >= config.search_budget_seconds
            or provider_requests >= config.max_requests
        ):
            hit_limit = True
            break
        ...
        while queue and len(current_batch) < min(
            config.concurrency,
            remaining_requests,
        ):
        ...
        attempt_budgets = _allocate_attempt_budgets(
            remaining_requests,
            len(current_batch),
        )
        ...
        with ThreadPoolExecutor(max_workers=len(current_batch)) as executor:
            future_map = {
                executor.submit(
                    ...
                    task,
                    started,
                    config.search_budget_seconds,
                    config.error_prefix,
                    max_attempts,
                ): task
                for task, max_attempts in zip(
                    current_batch,
                    attempt_budgets,
                    strict=True,
                )
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    results = []
                    attempts = 1
                    error_code = "BAIDU_SEARCH_UNEXPECTED_ERROR"
        ...
        for task, results, error_code in outcomes:
            ...
            useful_unbounded_branch = (
                task.range_start is not None
                or branch_yield >= config.stop_yield_ratio
            )
            if saturated and useful_unbounded_branch:
        ...
        if progress_callback is not None:
            query_count = len(
                {hit["query"] for hit in hits} | set(initial_queries)
            )
            progress_callback(
                {
    ...
    for retry_no in range(max_attempts):
        if time_module.monotonic() - scan_started >= search_budget:
            return (
                [],
                attempts,
                f"{error_prefix}_SEARCH_BUDGET_EXHAUSTED",
            )
        attempts += 1
        try:
