...
class BaiduSearchProvider:
    ...
    def __init__(self):
        ...
        self.auth_header = getattr(
            settings,
            "BAIDU_SEARCH_AUTH_HEADER",
            "Authorization",
        ).strip()
        if not self.api_key:
        ...
        self.cache_ttl = int(
            getattr(settings, "SEARCH_DISCOVERY_CACHE_TTL_SECONDS", 300)
        )
        ...
        for position, raw in enumerate(references, start=1):
            ...
            results.append(
                SearchResult(
                    ...
                    url=url,
                    website=clean_text(
                        raw.get("website") or raw.get("web_anchor"),
                        max_length=500,
                    ),
                    snippet=clean_text(
