...
def recover_stale_negative_index_scans(*, user=None, subject_id=None) -> int:
    total_budget = int(
        getattr(settings, "NEGATIVE_INDEX_TOTAL_TIMEOUT_SECONDS", 300)
    )
...
def _build_candidates(payload) -> list[dict]:
    minimum_relevance = int(
        getattr(settings, "NEGATIVE_INDEX_MIN_RELEVANCE_SCORE", 60)
    )
    minimum_rule_signal = int(
        getattr(settings, "NEGATIVE_INDEX_MIN_RULE_SIGNAL_SCORE", 25)
    )
    ...
    for record in payload.records.values():
        ...
        if (
            signal.score < minimum_rule_signal
            and source_type != "government_association"
        ):
            continue
        candidates.append(
            {
    ...
    for index, candidate in enumerate(candidates, start=1):
        ...
        candidate["verification_status"] = (
            NegativeIndexItem.VerificationStatus.NOT_REQUESTED
        )
...
def _verify_candidates(
    ...
    for candidate in verification_candidates:
        ...
        if not verification.succeeded:
            candidate["verification_status"] = (
                NegativeIndexItem.VerificationStatus.FAILED
            )
        ...
        verified_count += 1
        candidate["verification_status"] = (
            NegativeIndexItem.VerificationStatus.SUCCEEDED
        )
...
def _negative_items(candidates: list[dict]) -> list[dict]:
    minimum_negative = int(
        getattr(settings, "NEGATIVE_INDEX_MIN_NEGATIVE_CONFIDENCE", 60)
    )
    return [
        candidate
        for candidate in candidates
...
def _persist_result(
    *,
    ...
    high_risk_count = sum(
        1 for event in events if float(event["current_risk"]) >= 60
    )
    recent_cutoff = timezone.now() - timedelta(days=30)
    recent_count = sum(
        1
        for event in events
        if event["last_seen_at"] and event["last_seen_at"] >= recent_cutoff
    )
...
def execute_negative_index_scan(scan_id) -> dict:
    ...
    try:
        ...
        current_progress = (
            NegativeIndexScan.objects.filter(pk=scan.pk)
        ...
        NegativeIndexScan.objects.filter(pk=scan.pk).update(
            stage=NegativeIndexScan.Stage.VERIFYING
        )
        ...
        NegativeIndexScan.objects.filter(pk=scan.pk).update(
            stage=NegativeIndexScan.Stage.SCORING
        )
