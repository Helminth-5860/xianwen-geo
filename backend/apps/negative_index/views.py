...
ACTIVE_STATUSES = (NegativeIndexScan.Status.QUEUED, NegativeIndexScan.Status.RUNNING)
    ...
    def post(self, request, subject_id):
        try:
            with transaction.atomic():
                scan = create_negative_index_scan(user=request.user, subject_id=subject_id)
            ...
            return Response({"detail": "当前主体已有负面信息扫描正在进行。"}, status=HTTP_409_CONFLICT)
        return Response(NegativeIndexScanSummarySerializer(scan).data, status=HTTP_201_CREATED)
...
class SubjectNegativeIndexView(APIView):
    ...
    def get(self, request, subject_id):
        if not Subject.objects.filter(pk=subject_id, user=request.user).exists():
            return Response({"detail": "主体不存在。"}, status=HTTP_404_NOT_FOUND)
        recover_stale_negative_index_scans(user=request.user, subject_id=subject_id)
        scans = NegativeIndexScan.objects.filter(user=request.user, subject_id=subject_id)
        active = scans.filter(status__in=ACTIVE_STATUSES).order_by("-created_at").first()
        latest = scans.filter(status__in=COMPLETED_STATUSES).order_by("-finished_at", "-created_at").first()
        history = scans.filter(status__in=COMPLETED_STATUSES).order_by("-finished_at", "-created_at")[:30]
        return Response({
            "active_scan": NegativeIndexScanSummarySerializer(active).data if active else None,
            "latest_result": NegativeIndexScanDetailSerializer(latest).data if latest else None,
            "history": NegativeIndexScanSummarySerializer(history, many=True).data,
        })
...
class NegativeIndexScanDetailView(APIView):
    ...
    def get(self, request, scan_id):
        ...
        scan = NegativeIndexScan.objects.filter(pk=scan_id, user=request.user).first()
        if scan is None:
            return Response({"detail": "负面信息扫描记录不存在。"}, status=HTTP_404_NOT_FOUND)
...
class NegativeIndexEventListView(APIView):
    ...
    def get(self, request, scan_id):
        ...
        allowed_ordering = {"current_risk", "-current_risk", "last_seen_at", "-last_seen_at", "severity_score", "-severity_score", "evidence_score", "-evidence_score"}
        if ordering not in allowed_ordering:
            ordering = "-current_risk"
        queryset = queryset.order_by(ordering, "id")
        paginator = NegativeIndexPagination()
        page = paginator.paginate_queryset(queryset, request, view=self)
        return paginator.get_paginated_response(NegativeEventSummarySerializer(page, many=True).data)
...
class NegativeIndexEventDetailView(APIView):
    ...
    def get(self, request, event_id):
        event = NegativeEvent.objects.filter(pk=event_id, scan__user=request.user).select_related("scan").first()
        if event is None:
            return Response({"detail": "负面风险事件不存在。"}, status=HTTP_404_NOT_FOUND)
