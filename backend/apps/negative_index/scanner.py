    ...
    *,
    ...
    payload: AdaptiveSearchPayload = run_adaptive_search(
        ...
        config=AdaptiveSearchConfig(
            ...
            concurrency=int(
                getattr(settings, "NEGATIVE_INDEX_SEARCH_CONCURRENCY", 3)
            ),
            min_requests=int(getattr(settings, "NEGATIVE_INDEX_MIN_REQUESTS", 9)),
            stop_yield_ratio=float(
                getattr(settings, "NEGATIVE_INDEX_STOP_YIELD_RATIO", 0.08)
            ),
            low_yield_batches=int(
                getattr(settings, "NEGATIVE_INDEX_LOW_YIELD_BATCHES", 3)
            ),
