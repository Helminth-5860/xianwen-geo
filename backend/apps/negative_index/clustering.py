...
def title_similarity(left: str, right: str) -> float:
    ...
    jaccard = len(left_trigrams & right_trigrams) / max(
        1, len(left_trigrams | right_trigrams)
    )
    ...
    for item in ordered_items:
        ...
        for cluster in clusters:
            ...
            if item["category"] != head["category"]:
                continue
            if not _date_compatible(
                item.get("published_at"), head.get("published_at")
            ):
                continue
            similarity = max(
                title_similarity(
                    item.get("event_title", ""), head.get("event_title", "")
                ),
        ...
        if matched is None:
            ...
            published_key = (
                published_at.date().isoformat() if published_at else "unknown"
            )
            raw_key = (
                f"{item['category']}|"
                f"{_text(item.get('event_title') or item.get('title'))}|"
                f"{published_key}"
            )
...
def build_event(cluster: EventCluster) -> dict:
    ...
    domains = {
        row["root_domain"] for row in items if row.get("root_domain")
    }
