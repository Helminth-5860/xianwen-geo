...
class VerificationAnalysis(Protocol):
    @property
    def negative_confidence(self) -> int: ...

    @property
    def severity(self) -> int: ...

    @property
    def evidence_confidence(self) -> int: ...
...
def rule_signal(title: str, snippet: str) -> RuleSignal:
    ...
    best_category: str = NegativeEvent.Category.OTHER
    best_hits = 0
    ...
    *,
    ...
    if official and formal_category and signal.score >= 30:
        return CandidateAnalysis(
            ...
            event_title=title[:500],
            summary="权威政府/司法来源出现明确风险信号；智能分析暂时不可用时按保守官方证据规则识别。",
        )
    return CandidateAnalysis(
        ...
        event_title=title[:500],
        summary="智能判定暂时不可用，非官方信息不会仅凭关键词计入负面风险。",
    )
...
def analyze_candidates(
    ...
    payload_items = []
    verification_chars = int(getattr(settings, "NEGATIVE_INDEX_VERIFICATION_TEXT_CHARS", 4000))
        ...
        payload_items.append(
            {
                ...
                "verification_excerpt": candidate.get("verification_excerpt", "")[
                    :verification_chars
                ],
            }
        )
