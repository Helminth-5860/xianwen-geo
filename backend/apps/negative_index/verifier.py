...
def _evidence_excerpt(text: str, context: SubjectSearchContext) -> str:
    maximum = int(
        getattr(settings, "NEGATIVE_INDEX_VERIFICATION_TEXT_CHARS", 12000)
    )
    if len(text) <= maximum:
        return text
...
def verify_candidate(
    ...
    try:
        ...
        _title, text, _charset, _digest = parse_response(
            body=fetched.body,
        ...
        if not text.strip():
            return VerificationResult(
                False,
                "",
                "NEGATIVE_VERIFICATION_EMPTY",
            )
        return VerificationResult(
            True,
            _evidence_excerpt(text, context),
            "",
        )
