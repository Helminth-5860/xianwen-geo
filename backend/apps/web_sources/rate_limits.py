...
from .exceptions import WebSourceRateLimited, WebSourceUnavailable
from .url_security import fingerprint
...
def enforce_import_limits(*, request, user_id, subject_id, host: str) -> None:
    ...
    for label, raw, limit in dimensions:
        _increment(f"web-import:v1:limit:{label}:{fingerprint(label, raw)}", limit, window)
