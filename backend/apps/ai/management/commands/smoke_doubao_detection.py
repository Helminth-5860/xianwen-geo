...
from ...adapters.doubao import DoubaoDetectionAdapter
...
class Command(BaseCommand):
    help = "使用当前安全凭据和运行配置执行一次豆包 GEO 检测 smoke；不打印回答正文或密钥。"

    def add_arguments(self, parser):
        parser.add_argument("--model-key", default="doubao")
        parser.add_argument("--max-output-tokens", type=int, default=128)

    def handle(self, *args, **options):
        try:
            snapshot, resolved = resolve_detection_adapter(model_key=options["model_key"])
            if not isinstance(resolved, DoubaoDetectionAdapter):
                raise CommandError("Resolved adapter is not DoubaoDetectionAdapter.")
            adapter = resolved
            ...
            raise CommandError(f"Doubao smoke failed safely: {stable_code}") from exc

        self.stdout.write(
            self.style.SUCCESS(safe_smoke_summary(provider_key="doubao", response=response))
        )
