...
def test_plan_application_paths_are_canonical_with_subscription_activation_only():
    ...
    assert not any("package-applications" in path for path in paths)
    assert "/admin/plan-applications/{applicationId}/activate" in paths
    assert "post" in paths["/admin/plan-applications/{applicationId}/activate"]
