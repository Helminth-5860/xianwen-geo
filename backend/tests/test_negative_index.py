...
class NegativeIndexRuleTests(SimpleTestCase):
    ...
    def test_official_formal_signal_has_safe_fallback_when_ai_unavailable(self):
        ...
        self.assertIn("智能分析暂时不可用", fallback.summary)
        self.assertNotIn("AI", fallback.summary)

    def test_non_official_fallback_uses_customer_facing_chinese(self):
        signal = rule_signal(
            "网友称某公司服务体验较差",
            "用户发布投诉，但暂时没有权威结论。",
        )
        fallback = fallback_analysis(
            title="网友称某公司服务体验较差",
            snippet="用户发布投诉，但暂时没有权威结论。",
            source_type="forum_community",
            authority=40,
            signal=signal,
        )
        self.assertIn("智能判定暂时不可用", fallback.summary)
        self.assertNotIn("AI", fallback.summary)
