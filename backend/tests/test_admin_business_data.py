...
def make_superuser():
    return User.objects.create_superuser(phone="13900139000", nickname="超级管理员", password=PASSWORD)
...
def test_business_data_subject_search_is_superuser_only_and_fixed_to_twenty_per_page():
    ...
    customer = User.objects.create_user(phone="13800138000", nickname="排障客户", password=PASSWORD)
    subject_type = make_subject_type()
    for index in range(21):
        make_subject(user=customer, subject_type=subject_type, name=f"显问排障企业 {index + 1:02d}", index=index + 1)

    client = authenticate_admin_client(APIClient(), admin)
    response = client.get("/api/v1/admin/business-data", {"resource": "subjects", "q": "显问排障企业", "page": 1})
...
def test_business_data_rejects_ordinary_admin_even_with_valid_admin_session():
    ...
    response = authenticate_admin_client(APIClient(), profile.user).get("/api/v1/admin/business-data", {"resource": "subjects"})
...
def test_business_data_all_six_resource_queries_compile_and_validate_inputs():
    ...
    for resource, (queryset_factory, _) in RESOURCE_HANDLERS.items():
        ...
        response = client.get("/api/v1/admin/business-data", {"resource": resource, "q": "no-match-probe"})
        assert response.status_code == 200
        assert response.json()["data"]["items"] == []

    assert client.get("/api/v1/admin/business-data", {"resource": "unknown"}).status_code == 422
    assert client.get("/api/v1/admin/business-data", {"resource": "subjects", "page": 0}).status_code == 422
