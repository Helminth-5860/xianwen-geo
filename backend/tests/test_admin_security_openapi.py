...
def test_admin_security_openapi_marks_only_request_secrets_write_only():
    ...
    challenge = response_data["properties"]["challenge_id"]
    assert challenge.get("writeOnly") is not True
    assert response_data["required"] == ["requires_2fa"]


def test_admin_security_openapi_sms_code_pattern_requires_exactly_six_digits():
    schemas = specification()["components"]["schemas"]
    sms_code = schemas["AdminChallengeVerifyRequest"]["allOf"][1]["properties"]["sms_code"]
...
def test_admin_security_openapi_response_matrices_are_complete():
    ...
    expected = {
        ...
        ("/admin/auth/login/sms/send", "post"): {"200", "401", "403", "422", "429", "503", "500"},
        ("/admin/auth/login/sms/verify", "post"): {"200", "401", "403", "422", "503", "500"},
