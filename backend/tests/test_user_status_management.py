...
from apps.users.permissions import ActiveAccount
...
def create_user(phone="13800138000", **kwargs):
    created = User.objects.create_user(
        phone=phone,
        nickname=kwargs.pop("nickname", "账号用户"),
...
def test_notifications_are_isolated_and_read_requires_csrf_with_browser_session():
    ...
    for user, reason in ((first, "第一位用户原因"), (second, "第二位用户原因")):
        ...
        restored = admin_client.post(
            f"/api/v1/admin/users/{user.id}/unfreeze",
            {},
            format="json",
        )
    ...
    other = Notification.objects.filter(recipient=second).first()
    ...
    assert len(listing.json()["data"]["results"]) == 2
...
@pytest.mark.django_db
def test_admin_account_status_writes_require_real_csrf():
    admin = create_admin()
    target = create_user()
    admin_client = browser_client(admin.phone)
    freeze_path = f"/api/v1/admin/users/{target.id}/freeze"
...
@pytest.mark.django_db
def test_active_account_permission_matrix():
    ...
    request = Request()
    for account, active, expected in [
        ("active", True, True),
        ("frozen", False, False),
        ("cancel_pending", True, False),
    ]:
