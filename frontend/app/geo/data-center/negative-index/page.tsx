  ...
  if (result.status === "partial") {
    return (
      <Alert
        ...
        showIcon
        message="本次结果为部分结果"
        description="部分搜索、AI判定或高风险正文核验未完成。已保留可验证结果，但覆盖率可能低于正常扫描。"
      />
    );
  }
  return null;
...
export default function NegativeIndexPage() {
  ...
  }, [category, eventStatus, latestResult?.id, page]);
  ...
  const startScan = async () => {
    ...
    setError("");
    try {
      const scan = await startNegativeIndexScan(subjectId);
      setState((current) => ({
        active_scan: scan,
  ...
  const openDetail = async (event: NegativeEvent) => {
    ...
    try {
      setDetail(await getNegativeEvent(event.id));
    } catch (reason: unknown) {
      setError(userMessage(reason));
    } finally {
  ...
  const trend = useMemo(
    () => [...(state?.history ?? [])].reverse().slice(-20),
    [state?.history],
  );
    ...
    <main className={styles.page}>
      <div className={styles.header}>
        <div>
          <Space align="center">
            <WarningOutlined />
            <Title level={2} className={styles.title}>负面信息指数</Title>
            <Tooltip title="0–100 分，分数越高表示当前公开负面风险暴露越高。">
              <InfoCircleOutlined />
            </Tooltip>
          </Space>
          <Paragraph type="secondary" className={styles.subtitle}>
            基于百度公开搜索、来源可信度、AI语义判定和高风险证据核验，分析当前主体的公开负面风险。
          </Paragraph>
        </div>
        <Button
          type="primary"
        ...
        <Card className={styles.block}>
          <Space direction="vertical" size="small" className={styles.fullWidth}>
            <Space wrap>
              ...
              {activeScan.candidate_count ? <Text type="secondary">候选 {activeScan.candidate_count}</Text> : null}
            </Space>
        ...
        <>
          ...
          <Row gutter={[16, 16]} className={styles.block}>
            <Col xs={24} sm={12} xl={6}>
              <Card>
                <Statistic title="公开负面风险指数" value={riskNumber(latestResult.index_score)} precision={0} suffix="/ 100" />
                <Tag color={RISK_TAG_COLORS[latestResult.risk_level]}>{RISK_LABELS[latestResult.risk_level]}</Tag>
              </Card>
            </Col>
            <Col xs={12} sm={6} xl={6}>
              <Card><Statistic title="有效负面事件" value={latestResult.event_count} /></Card>
            </Col>
            <Col xs={12} sm={6} xl={6}>
              <Card><Statistic title="高风险事件" value={latestResult.high_risk_event_count} /></Card>
            </Col>
            <Col xs={12} sm={6} xl={3}>
              <Card><Statistic title="近30天" value={latestResult.recent_30d_event_count} /></Card>
            </Col>
            <Col xs={12} sm={6} xl={3}>
              <Card><Statistic title="正文核验" value={latestResult.verified_item_count} /></Card>
            </Col>
          </Row>

          <Row gutter={[16, 16]} className={styles.block}>
            <Col xs={24} lg={14}>
              <Card title="风险指数趋势" extra={<Text type="secondary">最近 {trend.length} 次扫描</Text>}>
                {trend.length > 1 ? (
                  <div className={styles.trend}>
                    {trend.map((scan) => {
                      const score = riskNumber(scan.index_score);
                      return (
                        <Tooltip key={scan.id} title={`${formatDateTime(scan.finished_at)} · ${Math.round(score)} 分`}>
                          <div className={styles.trendColumn}>
                            <div className={styles.trendBar} style={{ height: `${Math.max(3, score)}%` }} />
                          </div>
                        </Tooltip>
                      );
                    })}
                  ...
                  <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="完成至少两次扫描后显示趋势" />
                )}
              </Card>
            </Col>
            <Col xs={24} lg={10}>
              <Card title="风险类型构成">
                <Space direction="vertical" size="middle" className={styles.fullWidth}>
                  {latestResult.category_distribution.length ? latestResult.category_distribution.map((row) => {
                    if (!row.category) return null;
                    const percent = categoryTotal ? Math.round((row.count / categoryTotal) * 100) : 0;
                    return (
                      <div key={row.category}>
                        <div className={styles.distributionLabel}>
                          <Text>{CATEGORY_LABELS[row.category]}</Text>
                          <Text type="secondary">{row.count} · {percent}%</Text>
                        </div>
          ...
          >
            ...
            {eventCount > PAGE_SIZE ? (
              <div className={styles.pagination}>
                <Pagination current={page} pageSize={PAGE_SIZE} total={eventCount} showSizeChanger={false} onChange={setPage} />
              </div>
            ) : null}
          </Card>

          <Text type="secondary" className={styles.scanMeta}>
            最近扫描：{formatDateTime(latestResult.finished_at)} · 百度请求 {latestResult.provider_request_count} 次 · 原始结果 {latestResult.raw_result_count} · 独立结果 {latestResult.unique_result_count}
          </Text>
        </>
      ) : activeScan ? null : (
        <Card className={styles.block}>
          <Empty
            description={
              <Space direction="vertical">
                ...
                <Text type="secondary">首次扫描将使用百度公开搜索发现负面候选，再进行语义判定和事件去重。</Text>
              </Space>
            }
          >
            <Button type="primary" onClick={() => void startScan()} loading={starting}>开始首次扫描</Button>
          </Empty>
        </Card>
      )}
      ...
      >
        {detailLoading && !detail ? <Skeleton active /> : detail ? (
          <Space direction="vertical" size="middle" className={styles.fullWidth}>
            <Space wrap>
              <Tag>{CATEGORY_LABELS[detail.category]}</Tag>
              <Tag>{CLAIM_LABELS[detail.claim_type]}</Tag>
              <Tag>{STATUS_LABELS[detail.status]}</Tag>
              <Text strong>当前风险 {Math.round(riskNumber(detail.current_risk))}</Text>
            </Space>
            <Paragraph>{detail.summary || "暂无事件摘要。"}</Paragraph>
            <Row gutter={[12, 12]}>
              <Col span={6}><Statistic title="严重程度" value={detail.severity_score} /></Col>
              <Col span={6}><Statistic title="证据可信度" value={detail.evidence_score} /></Col>
              <Col span={6}><Statistic title="搜索可见度" value={detail.visibility_score} /></Col>
              <Col span={6}><Statistic title="关联信源" value={detail.source_count} /></Col>
            </Row>
              ...
              <Card size="small" key={source.id}>
                <Space direction="vertical" size={4} className={styles.fullWidth}>
                  <Space wrap>
                    ...
                    {source.verification_status === "succeeded" ? <Tag color="green">正文已核验</Tag> : null}
                    <Text type="secondary">证据可信度 {source.evidence_confidence}</Text>
                  </Space>
                  <Text>{source.title}</Text>
                  <Paragraph type="secondary" ellipsis={{ rows: 3, expandable: true }}>{source.snippet}</Paragraph>
